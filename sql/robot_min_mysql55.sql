-- Minimal robot tables for servers that do not have any robot schema yet.
...
CREATE TABLE IF NOT EXISTS robot (
    ...
    UNIQUE KEY uq_robot_object (object_id),
    UNIQUE KEY uq_robot_agent (agent_id),
    UNIQUE KEY uq_robot_name (name),
    KEY idx_robot_class (class_type, class_id),
    KEY idx_robot_map (loc_map, loc_x, loc_y),
    KEY idx_robot_ai (ai_enabled, deleted)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE IF NOT EXISTS robot_item (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (uid),
    KEY idx_robot_item_robot (robot_uid),
    KEY idx_robot_item_object (object_id),
    KEY idx_robot_item_item (item_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE IF NOT EXISTS robot_skill (
    uid BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT,
    robot_uid BIGINT(20) UNSIGNED NOT NULL DEFAULT 0,
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (uid),
    UNIQUE KEY uq_robot_skill (robot_uid, skill_id),
    KEY idx_robot_skill_id (skill_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE IF NOT EXISTS robot_ai (
    robot_uid BIGINT(20) UNSIGNED NOT NULL DEFAULT 0,
    object_id BIGINT(20) UNSIGNED NOT NULL DEFAULT 0,
    agent_id VARCHAR(64) NOT NULL DEFAULT '',
    ...
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (robot_uid),
    KEY idx_robot_ai_agent (agent_id),
    KEY idx_robot_ai_action (current_action),
    KEY idx_robot_ai_tick (last_tick_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS robot_log (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (uid),
    KEY idx_robot_log_robot (robot_uid, created_at),
    KEY idx_robot_log_type (log_type, created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
